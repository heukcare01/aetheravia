...
import OrderFilters, { OrderFilters as IOrderFilters } from '@/components/admin/OrderFilters';
...
export default function OrdersManagement() {
  ...
  const getCurrentFiltersFromURL = useCallback((): IOrderFilters => {
    if (!searchParams) {
      return {
        search: '',
        status: '',
  ...
  const updateURL = useCallback((filters: IOrderFilters) => {
  ...
  const fetchOrders = useCallback(async (filters: IOrderFilters) => {
    setLoading(true);
    setError('');
  ...
  const handleFiltersChange = useCallback((filters: IOrderFilters) => {
    updateURL(filters);
    fetchOrders(filters);
  ...
  const handleBulkDelete = useCallback(async (orderIds: string[]) => {
    if (!window.confirm(`Are you sure you want to delete ${orderIds.length} orders?`)) return;
    try {
      const response = await fetch('/api/admin/orders/bulk-update', {
        method: 'PUT',
        headers: {
          ...
          {selectedOrders.size > 0 && (
            <div className="flex flex-wrap gap-2">
              ...
              <button 
                ...
                onClick={() => handleBulkDelete(Array.from(selectedOrders))}
              >
                Delete Selected
              </button>
            </div>
