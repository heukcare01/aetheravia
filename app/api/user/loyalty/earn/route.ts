...
export async function POST(req: NextRequest) {
  ...
  const { points, description, orderId, source = 'order', txnId } = await req.json();
  if (!points || points <= 0) {
  ...
  // Optional: idempotency check (basic) – skip if txnId already exists
  ...
  emitAdminEvent({ ts: Date.now(), type: 'loyalty.earned', userId: String(user._id), delta: points, balance: user.loyaltyPoints, source, orderId });
